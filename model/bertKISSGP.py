...
class BertHF(Bert.modeling.BertPreTrainedModel):
    def __init__(self, config, n_dim, grid_bounds=(-10., 10.), grid_size=128, ard_num_dims=1):
        super(BertHF, self).__init__(config)
        # self.num_labels = num_labels
        self.bert = BertModel(config, n_dim=n_dim)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # self.classifier = nn.Linear(config.hidden_size, num_labels)
        self.gp_layer = GaussianProcessLayer(num_dim=n_dim, grid_size=grid_size, grid_bounds=grid_bounds, ard_num_dims=ard_num_dims)
        self.grid_bounds = grid_bounds
