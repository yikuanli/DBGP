...
import numpy as np
import torch
from model import Embedding, Linear
...
class BertHF(Bert.modeling.BertPreTrainedModel):
    def __init__(self, config, num_labels, n_dim, feature_dict=None):
    ...
    def forward(self, input_ids, age_ids=None, seg_ids=None, posi_ids=None, attention_mask=None, labels=None, beta=1):
        ...
        logits, kl_c = self.classifier(pooled_output)

        kl = kl+kl_c
        if labels is not None:
            loss = self.loss(logits.view(-1, self.num_labels), labels.view(-1, self.num_labels), kl, beta)
            return loss, logits
        else:
            return logits