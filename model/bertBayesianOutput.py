...
class BertHF(Bert.modeling.BertPreTrainedModel):
    def __init__(self, config, num_labels, n_dim):
        super(BertHF, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config, n_dim)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = Linear(n_dim, num_labels)
        self.apply(self.init_bert_weights)

        self.BayesLoss = BayesLoss(loss=nn.BCEWithLogitsLoss())
    ...
    def forward(self, input_ids, age_ids=None, seg_ids=None, posi_ids=None, attention_mask=None, labels=None, beta=1):
        ...
        logits, elbo = self.classifier(pooled_output)

        if labels is not None:
            loss = self.BayesLoss(logits.view(-1, self.num_labels), labels.view(-1, self.num_labels), elbo, beta)
            return loss, logits
        else:
            return logits